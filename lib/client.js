...
function GatewayCard(props) {
    ...
    }, React.createElement('span', { className: c.headText }, React.createElement('span', { className: c.name }, CARD_TITLE), React.createElement('span', { className: c.description }, CARD_DESCRIPTION)), dirty ? React.createElement('span', { className: c.pending }, '未保存') : null, chevron);
    const body = !open ? null : React.createElement('div', { className: c.body }, React.createElement('div', { className: c.status }, React.createElement('span', { className: c.dot, style: { background: statusDot } }), React.createElement('span', null, statusText), React.createElement('span', { className: c.muted }, `会话数 ${runtime?.sessions ?? 0}`), React.createElement('span', { className: c.muted }, runtime?.apiKeySet === true ? '· API 密钥已发放' : '· 尚未发放 API 密钥')), React.createElement('div', { className: c.mono }, `入口 ${prefix} · 流式 GET /sessions/:id/stream (SSE)`), !writable ? React.createElement('p', { className: c.error, role: 'status' }, '设置文档只读，无法保存更改') : null, ...FIELDS.filter((f) => f.kind === 'toggle').map((f) => {
        ...
        }), React.createElement('span', { className: c.label }, f.label), React.createElement('span', { className: c.hint }, f.hint));
    }), ...FIELDS.filter((f) => f.kind === 'select').map((f) => React.createElement('div', { key: f.key, className: c.field }, React.createElement('span', { className: c.label }, f.label), React.createElement('select', {
        className: c.input, disabled: !ready || !writable,
        value: String(value[f.key] ?? 'auto'),
        onChange: (ev) => select(f.key, ev.target.value),
    }, ...(f.options ?? []).map((o) => React.createElement('option', { key: o.value, value: o.value }, o.label))), React.createElement('span', { className: c.hint }, f.hint))), ...FIELDS.filter((f) => f.kind === 'text' || f.kind === 'number').map((f) => React.createElement('div', { key: f.key, className: c.field }, React.createElement('span', { className: c.label }, f.label), React.createElement('input', {
    ...
    }), React.createElement('span', { className: c.hint }, f.hint))), React.createElement('div', { className: c.field }, React.createElement('span', { className: c.label }, '管理密钥'), React.createElement('input', {
        type: 'password', className: c.input, placeholder: '留空不修改',
        disabled: !ready || !writable,
        value: draft('adminKey'),
        onChange: (ev) => setDraft('adminKey', ev.target.value),
    }), React.createElement('span', { className: c.hint }, '只写不回显；用于 /admin/* 与轮换')), freshKey !== null ? React.createElement('div', { className: c.mono }, '新 API 密钥: ', freshKey) : null, error !== '' ? React.createElement('p', { className: c.error, role: 'status' }, error) : null, React.createElement('div', { className: c.footer }, React.createElement('button', {
        type: 'button', className: c.ghost, disabled: rotating,
        onClick: () => void rotate(),
    }, rotating ? '轮换中…' : '轮换 API 密钥'), React.createElement('button', {
        type: 'button', className: c.discard, disabled: !dirty || saving,
        onClick: discard,
    }, '放弃更改'), React.createElement('button', {
