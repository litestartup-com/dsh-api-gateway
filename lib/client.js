...
const style = {
    ...
    field: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' },
...
function GatewayCard(props) {
    ...
    return React.createElement('div', { style: style.card }, React.createElement('div', { style: style.row }, React.createElement('strong', null, 'API Gateway'), React.createElement('span', { style: { ...style.dot, background: statusDot } }), React.createElement('span', null, statusText), React.createElement('span', { style: style.muted }, `会话数 ${runtime?.sessions ?? 0}`), runtime?.apiKeySet === true
        ? React.createElement('span', { style: style.muted }, '· API 密钥已发放')
        : React.createElement('span', { style: style.muted }, '· 尚未发放 API 密钥')), React.createElement('div', { style: style.mono }, `入口 ${prefix} · 流式 GET /sessions/:id/stream (SSE)`), !writable ? React.createElement('div', { style: { ...style.muted, color: 'var(--dsw-alias-state-error-primary)' } }, '设置文档只读，无法保存更改') : null, ...FIELDS.filter((f) => f.kind === 'toggle').map((f) => {
        const checked = value[f.key] === true;
        return React.createElement('div', { key: f.key, style: style.field }, React.createElement('input', {
            type: 'checkbox', style: style.toggle, checked, disabled: !ready || !writable,
            onChange: (ev) => toggle(f.key, ev.target.checked),
        }), React.createElement('span', { style: style.label }, f.label), React.createElement('span', { style: style.hint }, f.hint));
    }), ...FIELDS.filter((f) => f.kind === 'select').map((f) => React.createElement('div', { key: f.key, style: style.field }, React.createElement('span', { style: style.label }, f.label), React.createElement('select', {
        style: style.input, disabled: !ready || !writable,
        value: String(value[f.key] ?? 'auto'),
        onChange: (ev) => select(f.key, ev.target.value),
    }, ...(f.options ?? []).map((o) => React.createElement('option', { key: o.value, value: o.value }, o.label))), React.createElement('span', { style: style.hint }, f.hint))), ...FIELDS.filter((f) => f.kind === 'text' || f.kind === 'number').map((f) => React.createElement('div', { key: f.key, style: style.field }, React.createElement('span', { style: style.label }, f.label), React.createElement('input', {
    ...
    }), React.createElement('span', { style: style.hint }, f.hint))), React.createElement('div', { style: style.field }, React.createElement('span', { style: style.label }, '管理密钥'), React.createElement('input', {
        type: 'password', style: style.input, placeholder: '留空不修改',
        disabled: !ready || !writable,
        value: draft('adminKey'),
        onChange: (ev) => setDraft('adminKey', ev.target.value),
    }), React.createElement('span', { style: style.hint }, '只写不回显；用于 /admin/* 与轮换')), React.createElement('div', { style: style.row }, React.createElement('button', { type: 'button', style: style.button, disabled: saving || !ready || !writable, onClick: () => void saveAll() }, saving ? '保存中…' : '保存配置'), React.createElement('button', { type: 'button', style: style.button, disabled: rotating, onClick: () => void rotate() }, rotating ? '轮换中…' : '轮换 API 密钥')), freshKey !== null ? React.createElement('div', { style: style.mono }, '新 API 密钥: ', freshKey) : null, error !== '' ? React.createElement('div', { style: { ...style.muted, color: 'var(--dsw-alias-state-error-primary)' } }, error) : null);
}
