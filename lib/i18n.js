...
export const STRINGS = {
    en: {
        ...
        fields: {
            ...
            questions: { label: 'Answers ask_user_question', hint: 'gateway: relay to API clients (needs a free provider slot); host: leave it to the GUI' },
            approvals: { label: 'Decides permission prompts', hint: 'gateway: relay to API clients, for gateway-driven sessions only; host: leave them alone' },
    ...
    zh: {
        ...
        fields: {
            ...
            questions: { label: '谁回答互动提问', hint: 'gateway：转给 API 客户端答（需要 provider 槽位空着）；host：交给 GUI' },
            approvals: { label: '谁批授权请求', hint: 'gateway：转给 API 客户端批，只管本网关驱动的会话；host：不插手' },
...
export const dictFor = (candidates) => STRINGS[resolveLocale(candidates)];
/**
