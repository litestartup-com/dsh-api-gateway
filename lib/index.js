...
export default {
    ...
    apply(ctx, config) {
        ...
        ctx.effect(() => {
            ...
            // 用 disposed 标记处理卸载竞态。
            let disposeAnswerer = null;
            let disposed = false;
            void (async () => {
                const gateway = ctx.get('typertGateway', true);
                const connection = ctx.get('connection', true);
                let mounted = null;
                if (gateway?.wireStream?.open !== undefined && connection?.rpc?.call !== undefined) {
                    try {
                        mounted = await answerer.mountRemote({
                            ...
                            onFrame: (kind) => { answererStats.frames += 1; if (kind === 'waterfall')
                        ...
                        ctx.logger?.warn?.('[ohdsh-api-facade] remote event stream unavailable (' + String(error?.message ?? error) + ') — falling back to in-host waterfall listeners');
                    }
                }
                if (mounted === null) {
            ...
            return () => {
                ...
                if (disposeAnswerer !== null) {
                    try {
                        disposeAnswerer();
                    }
                    catch { /* noop */ }
                    ;
                    disposeAnswerer = null;
                }
                if (disposeRoute !== null) {
                    try {
                        disposeRoute();
