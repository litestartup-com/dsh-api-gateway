...
export default {
    ...
    apply(ctx, config) {
        ...
        const isAdmin = (req) => {
            if (config.adminKey === undefined || config.adminKey === '')
                return false;
        ...
        const createSession = async (body) => {
            if (apiSessions.size >= config.maxSessions)
                throw new Error(`session cap reached (${config.maxSessions})`);
            const options = {};
        ...
        const dispatch = async (req, res) => {
            ...
            if (seg.length === 3 && seg[0] === 'sessions' && req.method === 'GET' && (seg[2] === 'history' || seg[2] === 'stream')) {
                ...
                if (entry === undefined)
                    return sendJson(res, 404, { error: 'session_not_found', hint: `adopt the session first: POST ${config.prefix}/sessions/:id/adopt` });
