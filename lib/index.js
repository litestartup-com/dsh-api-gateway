...
import { HOST_DESCRIBE, invokeRemote, isMigrated, readHistory } from './adapter.js';
...
export default {
    ...
    apply(ctx, config) {
        ...
        // ---- proxy ----
        /** Generous: unary calls answer quickly, but a slow create must not hang. */
        const PROXY_TIMEOUT_MS = 60_000;
        /**
         * 0.1.2 起：已迁移方法经 typertGateway 进程内直调，网关自己造
         * server-response 信封（信封契约不变，manager 零感知）。
         * 未迁移方法诚实报 501——绝不静默走已失效的回环转发。
         */
        const proxyInProcess = async (req, res, method) => {
            ...
            if (method === 'session.history') {
                ...
                const gateway = ctx.get('typertGateway', true);
                ...
                try {
                    const value = await readHistory(gateway, payload.sessionId);
            ...
            try {
                const value = await invokeRemote(gateway, method, envelope.payload, AbortSignal.timeout(PROXY_TIMEOUT_MS));
                // 直播桥：会话经网关建立/唤醒时即入流（follow 流事件经 mux 广播）。
                if (method === 'session.create') {
                    const created = value;
                    if (typeof created.sessionId === 'string')
                        registry.ensure(created.sessionId);
                }
                if (method === 'session.prompt') {
                    const p = (envelope.payload ?? {});
                    if (typeof p.sessionId === 'string')
                        registry.ensure(p.sessionId);
                }
                return sendJson(res, 200, { type: 'server-response', rpcId, result: { ok: true, value } });
            }
            catch (error) {
         ...
         */
        const dispatchRespond = async (req, res) => {
        ...
        const answererStats = {
            ...
            waterfallEvents: [], lastBroadcastAt: 0, lastBroadcastSockets: 0,
        };
        /** 兜底 ctx.on 监听器的卸载器；远程路径就绪后替换。 */
        let disposeFallback = null;
        {
            const streamer = {
            ...
            registry = new FollowRegistry(streamer, (json) => {
            ...
            controlBridge = new ControlBridge(streamer, (json) => {
                for (const ws of outerSockets) {
                    if (ws.readyState === WebSocket.OPEN) {
                        try {
                            ws.send(json);
        ...
        /** Upstream liveness for /health: the in-process Remote dispatcher. */
        const healthUpstream = () => {
            return ctx.get('typertGateway', true) !== undefined ? 'ok' : 'unreachable';
        };
        // ---- HTTP dispatch ----
        const dispatch = async (req, res) => {
            ...
            if (seg.length === 0 && req.method === 'GET') {
                return sendJson(res, 200, {
                    ...
                    endpoints: [
                        ...
                        { method: 'GET', path: cfg.prefix + '/answerer/pending', auth: true, note: 'recovery channel: pending question/approval broadcasts (card-loss fix)' },
            ...
            // 卡片链(2026-09-17):manager 的挂起问答恢复通道——question/approval 帧
            // 只广播一次,断线窗口/manager 重启后卡片即永久丢失。本端点把仍挂起的
            // 载荷(rpcId + method + payload)按需交给 manager,由它重放/重建卡片。
            if (seg.length === 2 && seg[0] === 'answerer' && seg[1] === 'pending' && req.method === 'GET') {
                if (!requireAuth(req, res))
                    return;
                return sendJson(res, 200, { pending: answerer.pendingList() });
            }
                    ...
                    try {
                        const mounted = await answerer.mountRemote({
                            ...
                            sendResult: async (args, signal) => {
                                ...
                                if (!response.ok)
                                    throw new Error(`$events/result: HTTP ${response.status}`);
                            },
                            onFrame: (kind, event) => {
                                answererStats.frames += 1;
                                if (kind === 'waterfall') {
                                    answererStats.waterfalls += 1;
                                    if (typeof event === 'string' && answererStats.waterfallEvents.length < 8)
                                        answererStats.waterfallEvents.push(event);
                                }
                            },
                        });
                        if (disposed) {
