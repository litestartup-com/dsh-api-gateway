...
export const unaryProxyUrl = (proxyTarget, method) => `${proxyTarget.replace(/\/+$/, '')}/${method}`;
/** WebSocket URL of the mux downlink: ws(s)://<proxyTarget>/events.mux. */
export const muxProxyUrl = (proxyTarget) => `${proxyTarget.replace(/\/+$/, '').replace(/^http/, 'ws')}/events.mux`;
