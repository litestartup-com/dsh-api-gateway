...
function GatewayCard() {
  ...
  React.useEffect(() => {
    ...
    const refresh = () => {
      fetch(DEFAULT_PREFIX + '/health')
        ...
        .then((s: any) => { if (alive) { setState({ enabled: s.enabled === true, sessions: s.sessions ?? 0, apiKeySet: s.apiKeySet === true }); setError('') } })
        .catch(() => { if (alive) setError('网关未挂载（前缀与组合行配置不一致？）') })
    }
    refresh()
  ...
  const adminCall = async (path: string, body: unknown) => {
    setBusy(true)
    setError('')
    try {
      ...
      if (data.enabled !== undefined) setState((prev) => prev === null ? null : { ...prev, enabled: data.enabled })
    } catch {
      setError('管理请求失败')
    } finally {
      setBusy(false)
  ...
  return React.createElement('div', { style: style.card },
    React.createElement('div', { style: style.row },
      ...
      React.createElement('span', { style: { ...style.dot, background: on ? '#16a34a' : 'var(--dsw-alias-state-error-primary)' } }),
      React.createElement('span', null, on ? '已启用' : '已停用'),
      React.createElement('span', { style: style.muted }, `会话数 ${state?.sessions ?? 0}`),
    ),
    React.createElement('div', { style: style.mono },
      '入口: ', DEFAULT_PREFIX, ' · 流式: GET /sessions/:id/stream (SSE)'),
    state?.apiKeySet === true
      ? React.createElement('div', { style: style.muted }, 'API 密钥已发放（POST /key 领取）')
      : React.createElement('div', { style: style.muted }, '尚未发放 API 密钥'),
    freshKey !== null
    ...
    React.createElement('div', { style: style.row },
      React.createElement('input', {
        ...
        value: adminKey, onChange: (event) => setAdminKey((event.target as HTMLInputElement).value),
      }),
      React.createElement('span', { style: style.muted }, '停用/轮换需要管理密钥；仅 /health 在停用时仍可访问'),
    ),
